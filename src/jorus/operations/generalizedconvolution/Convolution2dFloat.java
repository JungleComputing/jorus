...
public final class Convolution2dFloat extends GeneralizedConvolution2d<float[]> {
	...
	protected void doItSS(float[] destination, float[] source, float[] kernel) {
		for (int y = 0; y < height; y++) {
			...
			// doKernelSS(destination,destinationIndex + x, source, sourceIndex
			// + x, kernel);
			// }
		}
	}

	private void doRowSS(float[] destination, int destinationIndex,
			float[] source, int sourceIndex, float[] kernel) {
		for (int x = 0; x < width; x++) {
			doKernelSS(destination, destinationIndex + x, source, sourceIndex
					+ x, kernel);
		}
	...
	private void doKernelSS(float[] destination, int destinationIndex,
			float[] source, int sourceIndex, float[] kernel) {
		float sum = 0;
		int kernelIndex = 0;
		...
		// sum /= kernel.length;
		destination[destinationIndex] = sum / kernel.length;
	}
	...
	// for (int j = 0; j < kernelHeight; j++) {
	// doKernelRowSS(destination, destinationIndex, source, sourceIndex + j *
	// rowSize, kernel, j * kernelWidth);
	// }
	// // destination[destinationIndex] /= (kernel.length);
	//
	// }
	//
	// private void doKernelRowSS(float[] destination, int destinationIndex,
	// float[] source, int sourceIndex, float[] kernel, int kernelIndex) {
	// for (int i = 0; i < kernelWidth; i++) {
	// destination[destinationIndex] += source[sourceIndex + i] *
	// kernel[kernelIndex + i];
	// }
	// }

	// @Override
	// protected void doItSS(float[] destination, float[] source, float[]
	// kernel) {
	// for (int y = 0; y < height; y++) {
	...
	protected void doItVS(float[] destination, float[] source, float[] kernel) {
		for (int y = 0; y < height; y++) {
			...
			int sourceIndex = y * rowSize;
			for (int x = 0; x < width; x++) {
				doKernelVS(destination, destinationIndex, source, sourceIndex,
						kernel);
	...
	private void doKernelVS(float[] destination, int destinationIndex,
			float[] source, int sourceIndex, float[] kernel) {
	...
	protected void doItVV(float[] destination, float[] source, float[] kernel) {
		for (int y = 0; y < height; y++) {
			...
			int sourceIndex = y * rowSize;
			for (int x = 0; x < width; x++) {
				doKernelVV(destination, destinationIndex, source, sourceIndex,
						kernel);
				destinationIndex += extent;
				sourceIndex += extent;
			}
	...
	private void doKernelVV(float[] destination, int destinationIndex,
			float[] source, int sourceIndex, float[] kernel) {
		float sum[] = new float[extent];
		int kernelIndex = 0;
		for (int j = 0; j < kernelHeight; j++) {
