...
public class Geometric2dFloat extends Geometric2d<float[]> {

	public Geometric2dFloat(Matrix backwardsTransformationMatrix,
			double[] translationVector, float[] background,
	...
	@Override
	public void doIt(float[] destination, float[] source) {

		int maxSourceX = linearInterpolation ? sourceWidth - 2
				: sourceWidth - 1;
		int maxSourceY = linearInterpolation ? sourceHeight - 2
				: sourceHeight - 1;

		for (int y = 0; y < destinationHeight; y++) {
			for (int x = 0; x < destinationWidth; x++) {
				int sourceX, sourceY;
				float sourceXf, sourceYf;
				double[] vArg;
				...
				sourceXf = (float) (vArg[0] / vArg[2]); /*
														 * homogeneous
														 * coordinates
														 */
				sourceYf = (float) (vArg[1] / vArg[2]);
				...
				if ((sourceX < 0) || ((int) (sourceXf + 0.5) > maxSourceX)
					...
					System.arraycopy(background, 0, destination,
							destinationIndex(x, y), extent);
				} else {
					// apply transformation on pixel (x,y) of the target image
					if (linearInterpolation) {
						int ul, ur, lr, ll, targetPixel;
						float alpha = sourceXf - sourceX;
						float beta = sourceYf - sourceY;
