...
public abstract class CommunicationUtil<T> {
	...
	public abstract void send(int dest, T data, int off, int len)
			throws IOException;

	public abstract void receive(int src, T data, int off, int len)
			throws IOException;

	public abstract void exchange(int partner, T out, int offOut, int lenOut,
			T in, int offIn, int lenIn) throws IOException;

	public void receive(int src) throws IOException {
		ReadMessage rm = system.receive(src);
		rm.finish();
	}

	public void send(int dest) throws IOException {
		WriteMessage wm = system.newMessage(dest);
		wm.finish();
	}

	public static CommunicationUtil<?> createImplementation(PxSystem system,
			Class<?> c) throws Exception {

		// FIXME: ugly -- J
		if (!c.isArray()) {
			throw new Exception("Illegal CommunicationUtil type: "
					+ c.getName());
		}
