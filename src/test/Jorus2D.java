...
public class Jorus2D {
	...
	private static final Logger logger = LoggerFactory.getLogger(Jorus2D.class);
	...
	private Jorus2D(String poolName, String poolSize, String fileName)
			throws Exception {
	...
	Array2dScalarDouble conv2D(Array2dScalarDouble source) throws Exception {
		double sx, sy;
		...
		for (int theta = MIN_THETA; theta < MAX_THETA; theta += STEP_THETA) {
			...
			for (sy = MIN_SY; sy < MAX_SY; sy += STEP_SY) {
				for (sx = MIN_SX; sx < Max_sx(sy); sx += Step_sx(sx)) {
					if (sx != sy) {
						filtIm1 = (Array2dScalarDouble) source.convGauss1x2d(sx, sy, -theta, 2, 3);
						filtIm2 = (Array2dScalarDouble) source.convGauss1x2d(sx, sy, -theta, 0, 3);
...
//				e.printStackTrace();
	...
	private void run() {
		...
		for (int i = 0; i < ITER; i++) {
			singleRun(array, master, "Jorus2D " + i);
	...
	static class ShutDown extends Thread {
		final Jorus2D server;

		ShutDown(Jorus2D server) {
			this.server = server;
		}
	...
	public static void main(String[] args) {
		...
		Jorus2D server = null;
		try {
			server = new Jorus2D(poolName, poolSize, fileName);
