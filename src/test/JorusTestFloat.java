...
public class JorusTestFloat {

	private static final Logger logger = LoggerFactory
			.getLogger(JorusTestFloat.class);
	...
	private JorusTestFloat(String poolName, String poolSize, String fileName)
			throws Exception {
	...
	private void run() {
		...
		try {
			PixelFloat pixel = new PixelFloat(1);
			array = new Array2dScalarFloat(128, 128, 0, 0, true);
			float[] pixelData = pixel.getValue();
			// pixelData[0] = 0.5;
...
//		array = (Array2dScalarDouble) array.convGaussAnisotropic2d(2, 0, 3, 11, 0, 3, 0.25 * Math.PI, true);
		array = (Array2dScalarFloat) array.convGauss1x2d(1, 1, 45, 2, 3);
...
//		array = (Array2dScalarDouble) array.convGauss2d(2, 0, 3, 2, 0, 3, false);
		array = (Array2dScalarFloat) array.clone(0, 0); // remove borders

		array = (Array2dScalarFloat) array.rotate(45, true, false, new PixelFloat(new float[] {0}));
...
//	}
	...
	static class ShutDown extends Thread {
		final JorusTestFloat server;

		ShutDown(JorusTestFloat server) {
			this.server = server;
		}
	...
	public static void main(String[] args) {
		...
		JorusTestFloat server = null;
		try {
			server = new JorusTestFloat(poolName, poolSize, fileName);
